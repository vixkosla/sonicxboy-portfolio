...
Публичный исходный код: https://github.com/vixkosla/sonicxboy-portfolio.
Продакшен можно по-прежнему выкладывать напрямую через Vercel CLI либо подключить
этот GitHub-репозиторий к Vercel. Перед деплоем убедиться, что рабочее дерево
чистое и `pnpm build` проходит.
